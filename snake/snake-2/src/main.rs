...
const GAME_WIDTH:i32 = 32; // ZX81 is 32 columns width
const GAME_HEIGHT:i32 = 24; // ZX81 is 24 lines height
...
impl Snake {
    ...
    ///
    fn r#move(&mut self, direction:i32, dt:i32) {
        // move body parts
        ...
        match direction {
            -1 => { // up
                head_y = (head_y - SNAKE_SPEED * dt).rem_euclid(GAME_HEIGHT) ;
            },
            1 => { // down
                head_y = (head_y + SNAKE_SPEED * dt).rem_euclid(GAME_HEIGHT);
            },
            -2 => { // left
                head_x = (head_x - SNAKE_SPEED * dt).rem_euclid(GAME_WIDTH);
            },
            2 => { // right
                head_x = (head_x + SNAKE_SPEED * dt).rem_euclid(GAME_WIDTH);
            },
...
impl Game {
    ...
    fn update(&mut self, dt:i32) {
        if !self.gameover {
            ...
            self.snake.r#move(self.direction, dt);
        }
    }
