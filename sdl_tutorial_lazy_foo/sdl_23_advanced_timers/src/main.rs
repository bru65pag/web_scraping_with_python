        ...
        match event {
            ...
            Event::KeyDown { timestamp: _, window_id: _,
                ...
                match keycode.unwrap() {
                    ...
                    keyboard::Keycode::P => {
                        *key_pressed = KeyPressed::Pause;
                    }
                    keyboard::Keycode::S => {
                        *key_pressed = KeyPressed::StartStop;
                    }
                    _ => {
                    }
                }
...
// print header text
    ...
    let surface = font.render("Press S to Start or Stop the timer")
    ...
    let target = Rect::new(i32::try_from(x).unwrap(),i32::try_from(y).unwrap(),width,height);
    canvas.copy(&texture, None, Some(target))?;

// print new millisec value
    let surface = font.render(&timer.get_ticks().to_string())
        .blended(Color::RGBA(0, 0, 0, 255))
        .map_err(|e| e.to_string())?;
    let texture_creator = canvas.texture_creator();
    let texture = texture_creator.create_texture_from_surface(&surface)
...
// initialize
    ...
    let window_size = WindowSize {height: 1200, width: 600};
    let window_location = WindowLocation {x:320, y: 300};
    let sdl_context = sdl2::init().unwrap();

    let video_context = sdl_context.video().unwrap();
    let window_context = video_context.window("Game",window_size.height,window_size.width)
    ...
    let mut key_pressed = KeyPressed::Null;

    let mut timer = Timer::new(&sdl_context);
...
// game loop
    while running {
        input(&sdl_context, &mut running, &mut background_color, &mut key_pressed);
        update(&mut timer,&mut key_pressed);
        render(&mut canvas_context, &font, &background_color, &mut timer)?;
        // Time management
